...
class Stage:
    def __init__(
        ...
        if not next_stage and not transitions:
           raise AssertionError("Either 'next_stage' or 'transitions' must be provided") 
        elif next_stage:
            get_next_stage = lambda *args: next_stage
        else:
            def get_next_stage(context: Context) -> str:
                *transitions, default_transition = self.transitions
        ...
        self.next_stages: list[str] = []
        if self.next_stage:
            self.next_stages.append(self.next_stage)
        for transition in self.transitions or []:
            self.next_stages.append(transition.to_stage)

    def __str__(self) -> str:
        return f"Stage({self.stage_id!r} => {self.next_stages!r})"
...
class Workflow:
    ...
    def _validate_stages(self) -> None:
        ...
        for stage in self.stage_id_to_stage_map.values():
            ...
            for transition in stage.transitions or []:
